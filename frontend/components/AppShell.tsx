...
const NAV = [
  ...
  { href: "/field/new", label: "New waybill" },
  { href: "/field", label: "Mobile pad" },
];
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  if (!user) {
    return <div className="grid min-h-screen place-items-center text-forest-800">Opening SafiRoute…</div>;
  }
  ...
  return (
    <div className="min-h-screen lg:grid lg:grid-cols-[260px_1fr]">
      <header className="flex items-center justify-between bg-forest-950 px-4 py-3 text-cream lg:hidden">
        ...
        <button
          type="button"
          className="tap rounded-lg border border-white/20 px-3 py-2 text-sm"
      ...
      <aside className={`${menu ? "block" : "hidden"} bg-forest-950 text-cream lg:flex lg:min-h-screen lg:flex-col`}>
        <div className="hidden items-center gap-3 border-b border-white/10 px-5 py-5 lg:flex">
          ...
          <div>
            ...
            <p className="mt-1 text-[11px] uppercase tracking-[0.18em] text-gold-400">Safisana Sales</p>
          </div>
        </div>
        ...
        <div className="mt-auto border-t border-white/10 p-4 text-sm">
          ...
          <p className="text-cream/50">Safisana Sales</p>
          <button
